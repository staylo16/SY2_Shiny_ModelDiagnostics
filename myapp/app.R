# Publishing code
#rsconnect::deployApp(appFiles=c('SY2_Explore_Model_Diagnostic_Plots.R',
#                                "md_app_datasets/toy_data_1.csv",
#                                "md_app_datasets/toy_data_2.csv",
#                                "md_app_datasets/toy_data_3.csv",
#                                "md_app_datasets/toy_data_4.csv",
#                                "md_app_datasets/toy_data_5.csv",
#                                "md_app_datasets/energy_data_cleaned.csv",
#                                "md_app_datasets/news_data_cleaned.csv",
#                                "md_app_datasets/mlb_data_cleaned.csv",
#                                "md_app_datasets/cric_bat_data.csv",
#                                "md_app_datasets/cric_bowl_data.csv",
#                                "md_app_datasets/nba_data_cleaned.csv"
#), appPrimaryDoc='SY2_Explore_Model_Diagnostic_Plots.R')
...
server <- function(input, output, session) {
 ...
 rvf_history_data <- reactiveValues(history = data.frame(x = numeric(), y = numeric(), is_recent = character()))
 
 # Clear dataframe when the sample counter is at 0
 observeEvent(resample$counter, {
  if (resample$counter == 0) {
   rvf_history_data$history <- data.frame(x = numeric(), y = numeric(), is_recent = character())
  }
 })
 ...
 # Empty dataframe to store history data
 sl_history_data <- reactiveValues(history = data.frame(x = numeric(), y = numeric(), is_recent = character()))
 
 # Clear dataframe when the sample counter is at 0
 observeEvent(resample$counter, {
  if (resample$counter == 0) {
   sl_history_data$history <- data.frame(x = numeric(), y = numeric(), is_recent = character())
  }
 })
 ...
 # Empty dataframe to store history data
 qq_history_data <- reactiveValues(history = data.frame(x = numeric(), y = numeric(), is_recent = character()))
 
 # Clear dataframe when the sample counter is at 0
 observeEvent(resample$counter, {
  if (resample$counter == 0) {
   qq_history_data$history <- data.frame(x = numeric(), y = numeric(), is_recent = character())
  }
 })
 ...
 history_data <- reactiveValues(history = data.frame(intercept = numeric(), slope = numeric(), is_recent = character()))
 
 # Clear dataframe when the sample counter is at 0
 observeEvent(resample$counter, {
  if (resample$counter == 0) {
   history_data$history <- data.frame(intercept = numeric(), slope = numeric(), is_recent = character())
  }
 })
  ...
  if (resample$counter > 0) {
   ...
   output$fittedLineHoriginal <- 
    ...
    output$fittedLineHqq <- renderPlot({
     req(resample$counter > 0)
     ggplot(data = data_minmax(), mapping = aes(x = .data[[input$x_var]], y = .data[[input$y_var]])) +
      geom_point(alpha = 0) +
 ...
 output$NumSamples <- 
  ...
  output$NumSamplesQQ <- renderText({
   req(resample$counter > 0)
   req(input$sample_size != "")
   paste("No. of generated data sets: ", resample$counter)
  })
